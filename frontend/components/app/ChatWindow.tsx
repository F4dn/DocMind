...
import { SourcesDrawer } from "./SourcesDrawer"; // ← new
...
export function ChatWindow({
  ...
  // ← new: open drawer when sources are clicked
  const handleShowSources = useCallback((srcs: SourceChunk[]) => {
  ...
  return (
    <>
      {/* Sources drawer — works on ALL screen sizes */}
      <SourcesDrawer
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="h-14 px-6 border-b border-white/5 flex items-center gap-3 flex-shrink-0 bg-base/40 backdrop-blur-xl">
            ...
            {/* Sources toggle button — visible when panel is hidden */}
            {activeSources.length > 0 && (
              <button
                onClick={() => setDrawerOpen(true)}
                className="xl:hidden ml-auto flex items-center gap-2 px-3 h-7 rounded-lg glass hover:border-brand-cyan/30 transition-all text-[10px] font-mono uppercase tracking-wider text-brand-cyan"
              >
              ...
              <div className="space-y-8">
                {messages.map((msg) => (
                  <MessageBubble
                    key={msg.id}
                    message={msg}
                    onShowSources={handleShowSources} // ← uses new handler
                  />
                ))}
              </div>
        ...
        {/* RIGHT — Citation panel (xl screens only, always in sync) */}
        <CitationPanel
          sources={activeSources}
          isStreaming={isStreaming}
          onOpen={() => setDrawerOpen(true)} // ← pass this down
        />
      </div>
    </>
  );
